<div class="span9">
	...
	<div id="claim_business" class="modal hide fade in" tabindex="-1" role="dialog" aria-labelledby="login" aria-hidden="false" >
		<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			<h3>Request Ownership for <span id="establishment_name_span"></span></h3>
		</div>
		<div class="modal-body">
			<form action="<?= base_url('claimBusiness') ?>" method="post" enctype="multipart/form-data" onsubmit="return validateForm()">
				...
				<div class="form-group">
					91<input type="text" placeholder="Contact (Mobile) number*" name="contact_number" value="<?= set_value('contact_number') ?>" required />
				</div>
				<div class="form-group">
	...
	<div class="row">	 
		...
		{ 
			...
			if ($merchant_detail['is_verified']) 
				$is_completed = '<img src="https://pngimage.net/wp-content/uploads/2018/06/verified-stamp-png-1.png" width="80px" align="left" />';
			else
				$is_completed = '<a href="javascript:void(0);" onclick="open_modal('.$merchant_detail['merchant_id'].',&apos;'.$merchant_detail['establishment_name'].'&apos;);" data-toggle="modal" class="btn btn-primary pull-right">Claim this business</a>';

			if ($merchant_detail['merchant_logo']) 
				$merchant_logo = '<img src="'.base_url(SELLER_ATTATCHMENTS_PATH.$merchant_detail['merchant_id'].'/'.$merchant_detail['merchant_logo']).'" width="80px" />&nbsp;&nbsp;&nbsp;';
			else
				$merchant_logo = '';

			echo '<div class="span9">
					<center><h4>
						'.$merchant_logo.ucfirst($merchant_detail['establishment_name']).$is_completed.
					'</h4></center>
							...
							<tr>
								<td>';
									foreach ($address['result'] as $addres) 
									{
										...
										$business_days = $addres['business_days'];
										$business_hours = $addres['business_hours'];
										...
										if ($business_days) 
											echo "<br />Business Days: ".$business_days;

										if ($business_hours) 
											echo "<br />Business Hours: ".$business_hours;
...
function validateForm() 
{
    ...
    var isValid = mobileValidation($("input[name='contact_number']").val());
    if (!isValid) 
    {
    ...
    //for address long
    var isValid = fileValidation($("input[name='file']").val(), true);
    if (!isValid) 
    {
        alert("Business Proof: PDF, JPG and PNG are allowed.");
        return false;
    }
